...
LOG_FILE = LOG_DIR / "case5_results.txt"
...
def build_packet(payload, sequence):
    ...
    crc = mavlink_crc(header + payload, MESSAGE_MAP[MSG_ID].crc_extra)
...
def main():
    ...
    with open(LOG_FILE, "w", encoding="utf-8") as log:
        log.write("MAVLink Test Case 5\n===================\nMSG_ID: 0\nPAYLOAD_PATTERN: ALL_FF\n")
        for payload_length in range(START_PAYLOAD_LENGTH, END_PAYLOAD_LENGTH + 1):
            test_id += 1
            payload = bytes([0xFF] * payload_length)
    ...
    sock.close()
    print(f"CASE 5 COMPLETED: {test_id}/{total_tests}")
