...
# Give SITL time to process one test
PROCESS_DELAY = 0.1
...
def build_mavlink2_packet(
    ...
    # Get CRC_EXTRA for known message IDs
...
def send_one_test(packet):
    ...
    try:
        ...
        print(
            f"[DEBUG] SOCKET ERROR: {type(e).__name__}: {e}"
        )
...
total_tests = (
    (END_MSG_ID - START_MSG_ID + 1)
    *
    (END_PAYLOAD_LENGTH - START_PAYLOAD_LENGTH + 1)
)
...
with open(
    ...
    for msg_id in range(
        ...
        for payload_length in range(
            ...
            # ------------------------------------------------
            # Generate reproducible random payload
            # ------------------------------------------------
            ...
            # ------------------------------------------------
            # Save complete human-readable result
            # ------------------------------------------------
            ...
            # ------------------------------------------------
            # Stop if SITL actually disappeared
            # ------------------------------------------------
